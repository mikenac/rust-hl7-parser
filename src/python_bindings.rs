...
use crate::types::{Hl7Component, Hl7Field, Hl7Message, Hl7Repetition, Hl7Segment};
...
fn parse(py: Python<'_>, message: &str, strict: bool) -> PyResult<Py<PyAny>> {
    ...
    match crate::parser::parse(message, mode) {
...
fn parse_file(py: Python<'_>, path: &str, strict: bool) -> PyResult<Vec<Py<PyAny>>> {
    ...
    let groups = crate::parser::group_message_lines(&content);
    let results = crate::parser::parse_message_groups(&groups, mode);
...
fn parse_batch(
    ...
    for (i, item) in messages.iter().enumerate() {
        ...
        match crate::parser::parse(msg, mode) {
